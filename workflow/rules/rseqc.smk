rule bam_stat:
    ...
    params:
        min_map_qual=config.get("params")
        .get("rseqc")
        .get("bamstat")
        .get("min_map_qual"),
    log:
        resolve_results_filepath(
            ...
            "logs/rseqc/bamstat/{sample}_bamstat.log",
        ),
    conda:
    ...
    resources:
        ...
        mem_mb=3000,
    message:
        "Executing RSeQC BAMSTAT with {threads} threads on the following files {input.bam}."
    shell:
        "bam_stat.py "
        "-i {input.bam} "
...
rule smatools_flagstat:
    ...
    resources:
        ...
        mem_mb=3000,
    message:
        "Executing SAMTOOLS FLAGSTAT with {threads} threads on the following files {input.bam}."
    shell:
        "samtools flagstat "
        "-i {input.bam} "
        ">& {output}"
...
rule rseqc_read_distribution:
    ...
    params:
        ...
        ref=resolve_single_filepath(
            config.get("resources").get("rseqc_data"),
            config.get("resources").get("refseq_bed"),
        ),
    log:
        resolve_results_filepath(
            config.get("paths").get("results_dir"),
            "logs/rseqc/read_distribution/{sample}_readdistribution.log",
        ),
    conda:
        resolve_single_filepath(
            config.get("paths").get("workdir"), "workflow/envs/rseqc.yaml"
    ...
    resources:
        ...
        mem_mb=3000,
    message:
        "Executing RSeQC READ DISTRIBUTION with {threads} threads on the following files {input.bam}."
    shell:
...
rule genebody_coverage:
    ...
    params:
        ...
        ref=resolve_single_filepath(
            config.get("resources").get("rseqc_data"),
            config.get("resources").get("housekeeping_genes"),
        ),
    log:
        resolve_results_filepath(
            config.get("paths").get("results_dir"),
    ...
    resources:
        ...
        mem_mb=3000,
    message:
        "Executing RSeQC Gene Body Coverage with {threads} threads on the following files {input.bam}."
    shell:
...
rule rseqc_junction_annotation:
    ...
    log:
        resolve_results_filepath(
            config.get("paths").get("results_dir"),
            "qc/rseqc/{sample}.junctionAnnotation.txt",
        ),
    conda:
        resolve_single_filepath(
            config.get("paths").get("workdir"), "workflow/envs/rseqc.yaml"
...
rule rseqc_junction_saturation:
    ...
    output:
        plotr=resolve_results_filepath(
            config.get("paths").get("results_dir"),
            "qc/rseqc/{sample}.junctionSaturation_plot.r",
        ),
    params:
        out_basename=lambda w, output: output[0].split(".", 1),
        ref=resolve_single_filepath(
    ...
    log:
        resolve_results_filepath(
            config.get("paths").get("results_dir"),
            "qc/rseqc/{sample}.junctionSaturation.txt",
        ),
    conda:
        resolve_single_filepath(
            config.get("paths").get("workdir"), "workflow/envs/rseqc.yaml"
...
rule rseqc_infer_experiment:
    ...
    log:
        resolve_results_filepath(
            config.get("paths").get("results_dir"),
            "qc/rseqc/{sample}.inferexperiment.txt",
        ),
    conda:
        resolve_single_filepath(
            config.get("paths").get("workdir"), "workflow/envs/rseqc.yaml"
...
rule rseqc_inner_distance:
    ...
    output:
        resolve_results_filepath(
            config.get("paths").get("results_dir"),
            "qc/rseqc/{sample}.inner_distance.txt",
        ),
    params:
        out_basename=lambda w, output: output[0].split(".", 1),
        ref=resolve_single_filepath(
    ...
    log:
        resolve_results_filepath(
            config.get("paths").get("results_dir"),
            "qc/rseqc/{sample}.innerdistance.txt",
        ),
    conda:
        resolve_single_filepath(
            config.get("paths").get("workdir"), "workflow/envs/rseqc.yaml"
...
rule rseqc_read_duplication:
    ...
    output:
        out1=resolve_results_filepath(
            config.get("paths").get("results_dir"),
            "qc/rseqc/{sample}.dup.pos.DupRate.xls",
        ),
    params:
        out_basename=lambda w, output: output[0].split(".", 1),
    log:
        resolve_results_filepath(
            config.get("paths").get("results_dir"),
            "qc/rseqc/{sample}.read_duplication.txt",
        ),
    conda:
        resolve_single_filepath(
            config.get("paths").get("workdir"), "workflow/envs/rseqc.yaml"
